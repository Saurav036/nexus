  ...
  constructor() {
    ...
    this.axiosInstance.interceptors.request.use(
      (config: InternalAxiosRequestConfig) => {
        ...
        // Uncomment to use access token instead
        // const accessToken = localStorage.getItem('auth0_access_token')
        // if (accessToken) {
        //   config.headers.Authorization = `Bearer ${accessToken}`
        // }

        // Uncomment to send both tokens
        // if (idToken) {
...
export const reportsApi = {
  ...
  getById: (id: number): Promise<ApiResponse<Report>> => {
    return apiService.get<ApiResponse<Report>>(`${API_ENDPOINTS.REPORTS}/${id}`)
  },

  getApiDetailsById: (id: number): Promise<ApiResponse<Report>> => {
    return apiService.get<ApiResponse<Report>>(`${API_ENDPOINTS.REPORTS}/${id}/api-details`)
  },
