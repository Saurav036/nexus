...
import { FiPlus, FiTrash2, FiEye, FiX, FiCopy, FiDownload } from 'react-icons/fi'
...
const ReportsList = () => {
  ...
  const fetchReports = async () => {
    ...
    try {
      setLoading(true)
      console.log('Fetching reports for org DB ID:', user.orgDbId)
      const response = await reportsApi.getByOrg(user.orgDbId)
      console.log('Reports API response:', response)
      // API returns 'result' field, not 'data'
      setReports((response as any).result || response.data || [])
    } catch (error) {
  ...
  const handleViewReportUrl = async (id: number) => {
    try {
      ...
      setDrawerOpen(true)
      const response = await reportsApi.getApiDetailsById(id)
  ...
  const handleDownloadReport = async () => {
    if (!selectedReportDetails?.apiUrl || !selectedReportDetails?.apiSecret) {
      toaster.create({
        title: 'Missing information',
        description: 'API URL or API Secret is missing',
        type: 'error',
        duration: 5000,
      })
      return
    ...
    try {
      ...
      await downloadReport(
        selectedReportDetails.apiUrl,
        selectedReportDetails.apiSecret,
      ...
      <Drawer.Root open={drawerOpen} onOpenChange={(e) => setDrawerOpen(e.open)} size="md" placement="end">
        ...
        <Drawer.Positioner>
          <Drawer.Content bg="white" color="gray.900">
            <Drawer.Header bg="white" borderBottomWidth={1} borderColor="gray.200">
              ...
              {loadingDetails ? (
                ...
                <Stack gap={6}>
                  ...
                  <Box>
                    ...
                    >
                      <Icon as={FiDownload} mr={2} fontSize="lg" />
