...
const ReportForm = ({ mode }: ReportFormProps) => {
  ...
  useEffect(() => {
    if (formData.connectionId) {
      ...
      setWorkbooks([])
      setViews([])
      // Only clear workbook/view if not in edit mode loading state
      if (!loading) {
  ...
  useEffect(() => {
    if (formData.connectionId && formData.workbookId) {
      ...
      setViews([])
      // Only clear view if not in edit mode loading state
      if (!loading) {
  ...
  const fetchWorkbooks = async (connectionId: number) => {
    try {
      ...
      // Handle both wrapped (response.data) and direct array responses
      const workbooksData = Array.isArray(response) ? response : (response.data || [])
  ...
  const fetchViews = async (connectionId: number, workbookId: string) => {
    try {
      ...
      // Handle both wrapped (response.data) and direct array responses
      const viewsData = Array.isArray(response) ? response : (response.data || [])
  ...
  const fetchReport = async (reportId: number) => {
    try {
      ...
      // Set form data - the useEffect hooks will automatically fetch workbooks and views
      setFormData({
