...
const ReportForm = ({ mode }: ReportFormProps) => {
  ...
  const fetchWorkbooks = async (connectionId: number) => {
    try {
      ...
      // Handle both wrapped (response.data/result) and direct array responses
      const workbooksData = Array.isArray(response) ? response : (response.data || response.result || [])
  ...
  const fetchViews = async (connectionId: number, workbookId: string) => {
    try {
      ...
      // Handle both wrapped (response.data/result) and direct array responses
      const viewsData = Array.isArray(response) ? response : (response.data || response.result || [])
