  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      if (user.role === 'ADMIN') {
        ...
        try {
          const connectionsResponse = await connectionsApi.getByOrg(orgDbId)
        ...
        {user && (
          <Box mb={6} bg="white" p={6} borderRadius="lg" borderWidth={1} borderColor="gray.200">
            <Flex justify="space-between" align="center" mb={4}>
              ...
              {user.role && (
                <Badge colorScheme={user.role === 'ADMIN' ? 'purple' : 'blue'} size="lg" px={3} py={1}>
                  {user.role}
                </Badge>
              )}
            </Flex>
              ...
              {recentReports.map((report) => (
                ...
                >
                  <Flex justify="space-between" align="center">
                    ...
                    <Badge colorScheme="green">{report.fileFormat}</Badge>
                  </Flex>
                </Box>
              ))}
            </VStack>
