...
const UsersList = () => {
  ...
  const fetchUsers = async () => {
    if (!user?.orgDbId) {
      console.log('⚠️ No orgDbId found for user')
      setLoading(false)
      return
    }
    ...
    try {
      setLoading(true)
      console.log('🏢 Fetching users for organization DB ID:', user.orgDbId)

      // Fetch users by organization using the numeric database ID
      const usersResponse = await usersApi.getByOrganization(user.orgDbId)
      console.log('📦 Users response:', usersResponse)

      const orgUsers = usersResponse.data || []
