...
const UsersList = () => {
  ...
  const fetchUsers = async () => {
    if (!user?.auth0Id) {
      console.log('⚠️ No auth0Id found for user')
      setLoading(false)
      return
    }
    ...
    try {
      setLoading(true)
      console.log('🔍 Fetching users for auth0Id:', user.auth0Id)
      ...
      if (!currentUserData.UserOrgMaps || currentUserData.UserOrgMaps.length === 0) {
        console.log('⚠️ No UserOrgMaps found for user')
        setUsers([])
        toaster.create({
      ...
      console.log('🏢 Fetching users for organization ID:', userOrgId)
